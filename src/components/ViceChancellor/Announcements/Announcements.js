...
function Announcements() {
  ...
  useEffect(() => {
    let getUserData = async () => {
      ...
      await axios
        .get(`http://localhost:5000/vice-chancellor/announcements`, {
          headers: {
            token: token,
          },
        })
  ...
  const setNewAnnouncements = async (e) => {
    if (e.target.value == "All") {
      ...
      setLoading(true);
      await axios
        .get(`http://localhost:5000/vice-chancellor/announcements/search`, {
          headers: {
            token: token,
          },
          params: {
  ...
  const deleteAnnouncement = async (id) => {
    await axios
      .delete(`http://localhost:5000/vice-chancellor/announcements/${id}`, {
        headers: {
          token: token,
        },
      })
  ...
  return (
    <>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
        ...
        <Link
          to={`/vice-chancellor/announcements/create`}
