...
function ViceChancellorLogin(props) {
  ...
  if (reactLocalStorage.get('token')) {
    if (reactLocalStorage.get("user_role") === "student") {
      ...
      navigate(`/teacher`);
    }else if(reactLocalStorage.get("user_role") === "vice-chancellor"){
      navigate(`/vice-chancellor`);
    }  
  }
},[])
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    axios
      .post("http://localhost:5000/vice-chancellor/login", formdata)
...
export default ViceChancellorLogin;
