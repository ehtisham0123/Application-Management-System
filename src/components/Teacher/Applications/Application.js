...
function Application() {
  ...
  const [isForwardedToVC, SetIsForwardedToVC] = useState("");
  ...
  useEffect(() => {
    let getUserData = async () => {
      ...
      await axios
        .get(`http://localhost:5000/teacher/applications/show/${id}`, {
          headers: {
            token: token,
          },
        })
        .then((response) => {
          if (response.data) {
            ...
            SetIsForwardedToVC(response.data.isForwardedToVC);
            setLoading(false);
          }
        })
        .catch((error) => {
  ...
  const approveApplication = async (id) => {
    await axios
      .put(
        `http://localhost:5000/teacher/applications/approve/`,
        { id: id },
        {
          headers: {
            token: token,
  ...
  const rejectApplication = async (id) => {
    await axios
      .put(
        `http://localhost:5000/teacher/applications/reject/`,
        { id: id },
        {
          headers: {
            token: token,
  ...
  const sendApplicationToVc = async (e) => {
    setSuccess("");
    setError("");
    await axios
      .post(
        "http://localhost:5000/teacher/forwarded_applications/vc/create",
        {
          application_id: id,
        },
        {
  ...
  const handleSubmit1 = async (e) => {
    ...
    await axios
      .post(
        "http://localhost:5000/teacher/remarks/create",
  ...
  const deleteRemark = async (id) => {
    await axios
      .delete(`http://localhost:5000/teacher/remarks/${id}`, {
        headers: {
          token: token,
        },
      })
  ...
  return loading ? (
    ...
    <>
      ...
      <div className="row mt-4 d-flex justify-content-center">
        {(teacher_role === "Chairman" && !isForwardedToVC) && (
          <div className="col-12 col-md-5 mb-3">
            <Button
              ...
              fullWidth
            >
              Forward Application to Vice Chancellor
            </Button>
          </div>
        )}
        {(isForwardedToVC) && (
          <div className="col-12 col-md-6">
            <div style={{
              background: "linear-gradient(45deg, #FFB300, #FF8E53)",
              borderRadius: "8px",
              padding: "9px",
              ...
              <span>
                This application is already forwarded to the Vice Chancellor.
              </span>
            </div>
          </div>
        )}
      ...
      >
        <DialogTitle id="alert-dialog-title">
          {"Are you sure you want to send this Application to Vice Chancellor"}
        </DialogTitle>
