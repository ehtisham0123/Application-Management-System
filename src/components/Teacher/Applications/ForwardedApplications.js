...
function ForwardedApplications() {
  ...
  useEffect(() => {
    ...
    let getUsersData = async () => {
      await axios
        .get(
          `http://localhost:5000/teacher/forwarded_applications`,
          {
            headers: {
              token: token,
            },
  ...
  const setNewApplications = async (e) => {
    if (e.target.value == "All") {
      ...
      setLoading(true);
      await axios
        .get(`http://localhost:5000/teacher/forwarded_applications/search-applications`, {
          headers: {
            token: token,
          },
          params: {
...
export default ForwardedApplications;
